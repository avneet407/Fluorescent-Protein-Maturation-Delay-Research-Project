...
def _simulate_true_states(F, n_steps, dt, u_base, u_amp, u_freq_cycles):
    """Propagates the true state forward through F alone (no process noise),
    driven by a synthetic sinusoidal gene-expression signal true_u, using
    whichever rows/cols of F carry state-from-state and state-from-u terms.
    Works for both the 3-state (1-step) and 4-state (2-step) systems.
    """
    n = F.shape[0] - 1  # number of maturation-pathway states (excludes u)
    t_true = np.arange(n_steps) * dt
    true_u = u_base + u_amp * np.sin(2 * np.pi * t_true / (n_steps * dt) * u_freq_cycles)

    true_states = np.zeros((n_steps, n))
    state = np.zeros(n)
    for k in range(n_steps):
        next_state = F[:n, :n] @ state + F[:n, n] * true_u[k]
        state = next_state
        true_states[k] = state

    return t_true, true_u, true_states
...
def run_kalman_1step(params):
    """Runs the 1-step Kalman filter against a freshly generated synthetic trace.

    `params` keys: km, kb, kd, alpha, dt, n_steps, qI, qM, qu, sigma_F,
    u_base, u_amp, u_freq_cycles, seed (optional).
    ...
    t_true, true_u, true_states = _simulate_true_states(
        F, n_steps, dt, params["u_base"], params["u_amp"], params["u_freq_cycles"],
    )
    true_I, true_M = true_states[:, 0], true_states[:, 1]

    rng = np.random.default_rng(params.get("seed"))
    z_n = alpha * true_M + rng.normal(0.0, params["sigma_F"], size=n_steps)

    x_est_all = _run_filter_loop(F, H, Q, R, z_n, n_states=3)
    I_est, M_est, u_est = x_est_all[:, 0], x_est_all[:, 1], x_est_all[:, 2]

    return {
        "is_two_step": False,
        "t": t_true, "z_n": z_n,
        "true_I": true_I, "true_M": true_M, "true_u": true_u,
        "I_est": I_est, "M_est": M_est, "u_est": u_est,
        "alpha": alpha, "max_F_diff": max_F_diff,
    }


def run_kalman_2step(params):
    """Runs the 2-step Kalman filter against a freshly generated synthetic trace.

    `params` keys: km1, km2, kb, kd, alpha, dt, n_steps, qI, qX, qM, qu,
    sigma_F, u_base, u_amp, u_freq_cycles, seed (optional).
    ...
    true_I, true_X, true_M = true_states[:, 0], true_states[:, 1], true_states[:, 2]

    rng = np.random.default_rng(params.get("seed"))
    z_n = alpha * true_M + rng.normal(0.0, params["sigma_F"], size=n_steps)

    x_est_all = _run_filter_loop(F, H, Q, R, z_n, n_states=4)
    ...
    return {
        "is_two_step": True,
        "t": t_true, "z_n": z_n,
        "true_I": true_I, "true_X": true_X, "true_M": true_M, "true_u": true_u,
